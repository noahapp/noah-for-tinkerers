...
> **Status/scope:** Doc 1 of 7 — as-shipped in the `noah` desktop app (`apps/desktop`). Describes the UI-tool protocol between the Rust agent runtime and the React renderer. Line numbers are approximate and were correct at the time of writing; treat symbol names as authoritative and re-grep if they drift. Positioning note: this document describes the open BYOK build only and makes no comparison to any hosted product.
...
Every assistant turn must be exactly one UI tool call (`apps/desktop/src-tauri/src/agent/prompts.rs`: "Every response MUST be exactly one of these tool calls"). The tools are registered in `register_ui_tools` (`apps/desktop/src-tauri/src/ui_tools.rs` ~line 504):
...
The central tool is `ui_spa`. Its schema (`ui_tools.rs` ~lines 222–268) is where the "slots not drawing" discipline is most visible:
...
`ui_user_question` (~lines 289–343) additionally enforces a *runtime* invariant the JSON schema alone can't: each question must have exactly one input mode — `options`, `text_input`, or `secure_input`. `ui_payload_from_tool_call` counts the modes and rejects zero or more-than-one (`ui_tools.rs` ~lines 106–120). `ui_info` and `ui_done` are minimal: a `summary_md` string, plus (for `ui_done`) the same optional `findings` shape as `ui_spa`.

One honesty note on the schema: `ui_spa` and `ui_done` set `additionalProperties: true` at the object root, while `ui_user_question` and `ui_info` set it `false`. So the enums and nested shapes are closed, but the two card tools tolerate extra top-level keys (they are ignored downstream rather than rejected).

## The TS payload types

`apps/desktop/src/lib/tauri-commands.ts` mirrors the Rust structs one-to-one so the boundary is typed on both sides. The discriminated union (~lines 130–167):
...
```
   ...
   │  • validate action_type ∈ {RUN_STEP, WAIT_FOR_USER}      │
...
Note there are two Rust stages, not one. `ui_payload_from_tool_call` (in `ui_tools.rs`) is the **validate-and-normalize** stage: it runs at tool-execution time, rejects bad enums/modes, tolerates the several field shapes models emit in the wild (flat `action_label`/`action_type`, hoisted `label`, or a legacy nested `action` object — `ui_tools.rs` ~lines 11–41), and produces one canonical JSON string. `parse_assistant_ui` (in `commands/agent.rs`) is the **parse-into-typed-struct** stage that reads that string back into `AssistantUiPayload`. It also carries a legacy text-marker fallback (`[SITUATION]…[PLAN]…[ACTION:…]`, `[DONE]`, `[INFO]`) for responses that arrive as plain text rather than a tool call (`agent.rs` ~lines 332–368).

## In the code

- **Schemas + runtime validation:** `apps/desktop/src-tauri/src/ui_tools.rs`
  - `ui_payload_from_tool_call` (~line 43) — the dispatcher; builds the canonical payload per tool name.
  - `action_type_valid` (~line 7) rejects anything but `RUN_STEP`/`WAIT_FOR_USER`; the one-input-mode check for `ui_user_question` is at ~lines 106–120.
  - `UiSpaTool::input_schema` (~line 222) — the quoted schema above; `UiDoneTool` (~line 393) reuses the `findings` shape.
  - Registration in `register_ui_tools` (~line 504); a test suite (~line 512) pins the enum/mode invariants.
- **Rust payload types + parse:** `apps/desktop/src-tauri/src/commands/agent.rs`
  - `AssistantSpaUi` (~line 88), `AssistantUiPayload` `#[serde(untagged)]` enum (~line 126), `SendMessageV2Result { text, assistant_ui }` (~line 135).
  - `parse_assistant_ui_json` matches on `kind` (~lines 197–199) and constructs the typed variant; `AssistantSpaUi.findings` carries a comment flagging that dropping findings here is a bug.
- **TS mirror types:** `apps/desktop/src/lib/tauri-commands.ts` — `AssistantUiPayload` union and members (~lines 110–167).
- **Deterministic render:** `apps/desktop/src/components/ChatPanel.tsx`
  - `renderFromUiPayload` (~line 1158) — `switch (ui.kind)` at ~line 1175 → `ActionCard` / `UserQuestionCard` / `DoneCard` / `InfoCard`; `default` falls back to a plain `MessageBubble`.
  - `apps/desktop/src/components/FindingsGrid.tsx` — `toneColor` map (~line 7) turns the closed `tone` enum into a CSS color; hard-caps to 6 tiles defensively (~line 32) even though the schema already enforces `maxItems: 6`.
  - `apps/desktop/src/components/StepsList.tsx` — renders the ordered `steps[]` with `status`-driven bullets.
- **The prompt that steers the model to think in slots:** `apps/desktop/src-tauri/src/agent/prompts.rs`, the "UI Tool Calls" section (~line 36). It repeats the slot discipline verbatim and includes an explicit anti-pattern:
